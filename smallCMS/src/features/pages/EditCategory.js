...
function EditCategory(){
    ...
    useEffect(() => {
        ...
        if (selectedCategory?.icon || selectedIcon) {
            ...
            } else if (selectedCategory.icon === 'GiStrong' || selectedIcon === 'GiStrong') {
    ...
    <Link to="/categories" className="header-button1">Go back</Link>
    </div>
     <form className={"container"} onSubmit={handleSubmit}>
        ...
        <div>
          ...
          />
             <div className="image-container" style={{ display: 'flex', justifyContent: 'center', marginLeft: "5%" }}>
                 <img src={selectedImage ? selectedImage : selectedCategory?.photoUrl} alt="Uploaded file" style={{ width: '200px' }} />
            </div>
        </div>
         <label htmlFor="categoryIcon">Category Icon:</label>
            {selectedCategory?.icon && !selectedIcon  && getIconComponent(library, selectedCategory?.icon)}
        <button type="button" onClick={() => setIsIconListVisible(!isIconListVisible)} style={{maxWidth: 250}}> Icon List</button>
        {isIconListVisible && ( 
            <div
            style={{
                display: 'flex',
        ...
        <div>
        {selectedIcon && getIconComponent(library, selectedIcon)}
        </div>
